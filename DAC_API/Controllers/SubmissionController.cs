...
namespace DAC_API.Controllers {
    ...
    public class SubmissionController : ControllerBase {
        ...
        public async Task<ActionResult<SubmissionResponseDTO>> CreateSubmission(CreateSubmissionDTO createSubmissionDto) {
            ...
            try {
                var user = await _context.Users.FindAsync(createSubmissionDto.UserId);
                if (user == null) {
                    return BadRequest($"User with ID {createSubmissionDto.UserId} not found");
                }

                var challenge = await _context.Challenges.FindAsync(createSubmissionDto.ChallengeId);
                if (challenge == null) {
                ...
                if (challenge.Points.HasValue) {
                    if (!user.Points.HasValue)
                        user.Points = 0;

                    user.Points += challenge.Points.Value;
